...
const Trade = () => {
    ...
    const handlePairChange = async (event) => {
        ...
        // Mock API call
        const response = await fetch(`https://fcsapi.com/api-v3/forex/latest?id=${selectedSymbol}&access_key=mSoo360DFh75rFyR677Ov`);
        const data = await response.json();
        const res =await data.response[0]
        console.log(data)
        if(data.code===200){
        setBuyPrice(res.c);
        setSellPrice(res.l);
        }else{
            alert("Try after a minute")
        }
    };

    const handleBuy = () => {
        alert(`Buying ${selectedPair} at ${buyPrice}`);
        // Add logic to handle buying the pair
    };

    const handleSell = () => {
        alert(`Selling ${selectedPair} at ${sellPrice}`);
        // Add logic to handle selling the pair
    };
    ...
    return (
        ...
        <div className="trade-container">
            ...
            <div className="dropdown-container">
                <label htmlFor="tradingPair">Select Trading Pair:</label>
                <select id="tradingPair" onChange={handlePairChange} value={selectedPair}>
            ...
            {selectedPair && (
                <div className="price-container">
                    <div className="price-info">
                        <span>Buy Price: {buyPrice !== null ? `$${buyPrice}` : 'Loading...'}</span>
                    </div>
                    <div className="price-info">
                        <span>Sell Price: {sellPrice !== null ? `$${sellPrice}` : 'Loading...'}</span>
                    </div>
                    <div className="buttons-container">
                        <button onClick={handleBuy} disabled={!buyPrice}>Buy</button>
                        <button onClick={handleSell} disabled={!sellPrice}>Sell</button>
                    </div>
                </div>
            )}
        </div>
